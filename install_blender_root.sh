...
install_blender() {
    log "Extracting Blender to ${INSTALL_DIR}/${BLENDER_VERSION}..."
     mkdir -p "$INSTALL_DIR"
     tar -xf "$TMP_DIR/${BLENDER_TARBALL}" -C "$INSTALL_DIR"
     mv "$INSTALL_DIR/blender-${BLENDER_VERSION}-linux-x64" "$INSTALL_DIR/${BLENDER_VERSION}"

    log "Creating symlink ${BIN_SYMLINK}..."
     ln -sf "${INSTALL_DIR}/${BLENDER_VERSION}/blender" "$BIN_SYMLINK"
}
