...
from kernel_bench import summarize
...
def run(args, row):
    ...
    with torch.inference_mode(), precision:
        ...
        try:
            ...
            for _ in range(args.iterations):
                output, elapsed = timed_call(lambda: model(inputs), torch.cuda.synchronize)
    ...
    row['images_per_joule'] = args.batch_size * args.iterations / row['energy_j'] if row.get('energy_j') else None
    row.update(status='ok', images_total=args.batch_size * args.iterations,
...
def main(argv=None):
    ...
    row = dict(benchmark_schema_version=2, suite='vision_infer', status='failed',
               ...
               mode=args.mode, seed=SEED, timing_method='synchronized_iteration_v2')
    try:
